function [sig, Fs, start, SigAlone, NoiseAlone, Noise2Alone] = ...
    AddNoiseReturnSecond(SignalWav, NoiseWav, Noise2Wav, MaskerWavStart, ...
                snr, duration, fixed, in_rms, out_rms, warning_noise_duration, HRIRmatFile, Azimuths)
%
% Many modifications September 2014 for correct function when HRTFs are indicated
%
% from Version 7.1 of add_noise
...
%   Also return another noise from a different file, matched in level
%	The level of the noise can be fixed, and the output level can be normalised
%	Randomize the starting point of the noise if a starting point is not given
%
%  'SignalWav' - the name of a .wav file containing the signal or target
%  'NoiseWav' - the name of a .wav file containing the noise
%       (must be longer in duration than signal)
%  'Noise2Wav' - the name of a .wav file containing the noise that is returned separately
%  MaskerWavStart - a 2-element vector
...
%                 snr, duration, fixed, in_rms, out_rms, warning_noise_duration,Azimuths)
%
% 	Combine a noise and signal waveform at an arbitrary signal-to-noise ratio
...
% Version 2.0 modified from combine.m (December 1999) Isaac -- December 2001
...
RISE_FALL = 10;     % taper the noise on and off, adding this duration
                    %           to start and finish of signal (was 200)
                    % This is in addition to the warning_noise_duration
                    % changed to 10 ms on 21 April 2009

%% initialisation for binaural signals
BinauralSignals=0;
if nargin==11 && ~strcmp(HRIRmatFile,'none')
    BinauralSignals=1;
end
...
if StereoTarget~=StereoMasker
    error('Both target and two maskers must be consistently mono or stereo.');
end
...
nz_samples=[length(nz) length(nz2)];
...
if nz_samples(1)<n_samples || nz_samples(2)<n_samples
   error('The noise waveform(s) is/are not long enough.');
end

%% select a random portion of the noise file, of the appropriate length
start=zeros(1,2);
for i=1:2
    if MaskerWavStart(i)<1
        start(i) = floor((nz_samples(i)-n_samples)*rand);
    else
        start(i)=MaskerWavStart(i);
    end
end

% copy the original noise waveforms
noise = nz;
noise2 = nz2;
...
noise2(1:start(2)-1,:)=[];
noise2(n_samples+1:length(noise2),:)=[];

%% put rises and falls on to the noises/maskers
noise=NewTaper(noise, RISE_FALL, RISE_FALL, Fs);
noise2=NewTaper(noise2, RISE_FALL, RISE_FALL, Fs);
% extra copy of noise
NoiseAlone=noise;
Noise2Alone=noise2;
...
rms_noise2 = rms(noise2(genuine_signal));
...
elseif strcmp(fixed, 'noise') % fix the noise level and scale the signal
   if in_rms>0 % scale the noise to the desired level, then scale the level of the signal and add it to the noise
      ...
      Noise2Alone = Noise2Alone * in_rms/rms_noise2;
      ...
      % but always scale 2nd noise to level of the 1st
      Noise2Alone = Noise2Alone * rms_noise/rms_noise2;
   end
else
...
if (out_rms>0)
   ...
   Noise2Alone = Noise2Alone * out_rms/rms_total;
end
...
Noise2Alone = Noise2Alone * 10^(-correction/20);
